# -*- coding: utf-8 -*-
"""
Created on Sat Jun 24 20:31:21 2023
...
def finetune_double(net, trainset, trainloader, valset, valloader, out_channels, epochs, checkpoint_count=2) :
    ...
    f_t = open(f"loss{out_channels}_double_train_fine.txt", "w")
    f_v = open(f"loss{out_channels}_double_val_fine.txt", "w")
    ...
    for epoch in range(epochs):  # loop over the dataset multiple times
        ...
        with tqdm(total=len(trainset), desc ='Epoch: '+str(epoch)+"/"+str(epochs), unit='img') as prog_bar:
            ...
            if epoch % checkpoint_count == 0 :
                torch.save(net.state_dict(), MODEL_PATH + f'checkpoint_ch{out_channels}_double_{epoch}_fine')
            running_loss /= (i+1) 
...
if __name__ == "__main__":
    ...
    for out_channels in out_channels_list :
        #if train_double_net :
            net = UNetDouble(in_channels=in_channels, out_channels=out_channels, depths=[32, 64, 128, 256])
            net.load_state_dict(torch.load(MODEL_PATH + f'checkpoint_ch{out_channels}_double_20'))
            net = net.cuda().float()
            summary(net, (in_channels, 176, 64), device='cuda')
            
            net = finetune_double(net, trainset, trainloader, valset, valloader, out_channels, EPOCHS) 
            torch.save(net.state_dict(), MODEL_PATH + f'model_ch{out_channels}_double_final_fine')
        #else :
